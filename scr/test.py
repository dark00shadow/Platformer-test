...
Block8 = pyglet.sprite.Sprite(pyglet.image.load('32x32-white.png') ,x=228,y=132, batch=ObjectBatch)
...
def BlockSolid(BlockX, BlockY):
    ...
    if collision.rectangle(Player.x,Player.y ,BlockX+5,BlockY+30 ,32,32 ,22,2):
        ...
        if KeyHandler[key.W]:
            ...
            Jump = True
    if collision.rectangle(Player.x,Player.y ,BlockX+5,BlockY ,32,32 ,22,2):
        Player.y -= 2
        Jump = False
...
def Update(dt):
    ...
    if Jump == True:
        Player.y += 3
        if Player.y >= PlayerOldPosX + 200: Jump = False
